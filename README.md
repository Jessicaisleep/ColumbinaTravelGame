...
┌────────────────┬──────────────────┬──────────────────┐
│  半截面屋子      │   院子            │   花园            │
│  书架/床/桌/灶台 │  房屋正门与前庭     │  树/花丛/池塘      │
│  墙上有搁板      │  从底栏进入田地     │  玩具摆在这里      │
│  （切开一半，能  │  独立场景           │                  │
│   看见她在里面） │                   │                  │
└────────────────┴──────────────────┴──────────────────┘
        纳西妲在里面实时走动、做事
   ...
   │        │ 它滚到田里去了。│  ← 她说话只从气泡出   │
   │        ╰───────┬──────╯                       │
   │  [送她出门] [田地] [厨房 10] [玩具 8] [仓库 36] …│  ← 按钮也在画面里
   └──────────────────────────────────────────────┘
```
...
**田地是独立的 4:3 场景**，从家园底栏进入。背景中的四块旱田和两块水田都可单独种植；哥伦比娅在家时也会跟着进入田地。
...
**没有单文件版** —— 代码 39 个文件 + 素材 40 张图，直接整个 `columbina-travel` 文件夹拷给别人就能玩。
