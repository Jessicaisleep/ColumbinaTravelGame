...
(function (root) {
  ...
  /**
   * 玩具落位槽。**一共 4 个，同时也就决定了"最多同时摆 4 件"**（摆放、自动补位、
   * 界面文案都读这张表的长度）。
   *
   * 为什么是 4 个（不是更多）：
   *   · 主角要站到玩具旁边玩，来访的同伴也要玩 —— 院子里同时有人有玩具
   *   · 同伴的立绘有些比主角宽，需要的空档更大
   *   · 最宽的吊床（约 0.95 身高、1.55:1）单是半宽就有 0.10 屏宽；摆 5 件时行距
   *     只能压到 0.14，她站过去就会踩到/压住旁边那件（自检里有专门的断言盯着）
   *   · 4 件能把行距拉到 0.20–0.28，上面这些问题就都不存在了
   *
   * 摆法：左右各一件靠里、一件靠前，中央正门与通道留空（避免玩具遮住主角）；
   * 外侧槽位也给最大尺寸的吊床留出了完整显示空间。
   */
  NT.data.toySlots = [
    { x: 0.115, y: 0.820, area: 'left',  place: 'outdoor' },
    { x: 0.330, y: 0.960, area: 'left',  place: 'outdoor' },
    { x: 0.680, y: 0.960, area: 'right', place: 'outdoor' },
    { x: 0.885, y: 0.820, area: 'right', place: 'outdoor' }
  ];

  /**
   * 后院的玩具落位槽。后院是另一张背景图，所以槽位单独一套。
   *
   * 数量跟着上限走（= toySlots 的长度），排成**同一排、行距 0.24**：
   *   · 同一排 -> 玩具之间只比宽度，最宽的一对（吊床 + 月灵琴）半宽和约 0.16，
   *     0.24 的行距留得很足，不会互相压住
   *   · 都在同一排 -> 她"站在玩具跟前"时脚下不会落进别的玩具的地盘里
   *     （落点由 home.backyardSpot 算，见那里的说明）
   */
  NT.data.backyardSlots = [
    { x: 0.140, y: 0.740, area: 'left',  place: 'outdoor' },
    { x: 0.380, y: 0.740, area: 'left',  place: 'outdoor' },
    { x: 0.620, y: 0.740, area: 'right', place: 'outdoor' },
    { x: 0.860, y: 0.740, area: 'right', place: 'outdoor' }
  ];
